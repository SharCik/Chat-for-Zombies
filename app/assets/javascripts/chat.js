...
var ready = function () {
    ...
    chatBox = {
        ...
        restructure: function () {    /*Сдвиг чатов относительно друг друга.*/
            align = 0;
            for (x in chatBoxes) {
                chatbox_id = chatBoxes[x];

                if ($("#chatbox_" + chatbox_id).css('display') != 'none') {        /*Проверка открыт ли чат.*/
                    if (align == 0) {                                              /*Для первого чата*/
        ...
        createChatBox: function (conversation_id, minimizeChatBox) {
            if ($("#chatbox_" + conversation_id).length > 0) {
                if ($("#chatbox_" + conversation_id).css('display') == 'none') {         /*Проверка  на наличие открытых чатов с данным юзером.*/
                    $("#chatbox_" + conversation_id).css('display', 'block');            /*Если нету , то создание блочного элемента*/
                ...
                var count = chatBoxes.length;

            if (count == 0 || count%2 == 0){                                              /*Для первого чата*/
                $("#chaty1").append('<div id="chatbox_' + conversation_id + '" class="chatbox"></div>')
            } else {                                                            
                $("#chaty2").append('<div id="chatbox_' + conversation_id + '" class="chatbox"></div>')
            }
            ...
            for (x in chatBoxes) {                                                          /* Прибавляем созданный */
            ...
            chatBoxes.push(conversation_id);                                                /* Добавляем в массив с чатами , тот который открыли */
